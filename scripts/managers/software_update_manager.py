...
class SoftwareUpdateManager:
    ...
    def reinstall_missing_buckets(self):
        ...
        for bucket_name, software_list in missing_bucket_names.items():
            self.logger.info(f"Bucket [{colored(bucket_name, 'yellow')}] not found, will re-install it now...")

            if self.logger.is_debug():
                self.logger.debug("The following software was found in the bucket:")

                for software in software_list:
                    self.logger.debug(f"- [{colored(software, 'yellow')}]")

            self.install_bucket(bucket_name)
            self.logger.info(f"Successfully installed bucket [{colored(bucket_name, 'yellow')}]!")
    ...
    def register_service(self):
        ...
        self.logger.info("Registered updater service started successfully!")
    ...
    def check_for_scoop_updates(self):
        self.logger.info("Checking for newer scoop updates...")
        ...
        if "Scoop out of date." in output:
            self.logger.info("Updating scoop since it's out of date...")
            ...
            self.logger.info("Successfully updated scoop")
    ...
    def check_for_updates(self):
        ...
        if not self.outdated_software:
            self.logger.info("Nothing to update, skipping...")
        else:
    ...
    try:
        ...
        while True:
            if not manager.check_for_connectivity():
                ...
                logger_reference.info(f"Established connection now! Proceeding with software update manager cycle...")
